		...
		try {
			int returnCode = PlatformUI.createAndRunWorkbench(display, new WorkbenchAdvisor() {
				...
				public String getInitialWindowPerspectiveId() {
					return "com.localllama.plugin.ui.LocalLlamaPerspective";
				}
			});
			...
			display.asyncExec(() -> {
				...
				if (window != null) {
					...
					if (page != null) {
						try {
							page.showView("com.localllama.plugin.view.LocalLlamaView");
						} catch (Exception e) {
							System.err.println("Failed to open LocalLlamaView: " + e.getMessage());
						}
					}
				}
			});
